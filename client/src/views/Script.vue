<template>
    <div class="mx-auto my-auto" v-if="script_loaded === false" >
        <button   class="btn btn-primary btn-lg" @click="LoadScript">Запустить скрипт</button>
    </div>
    <div v-else class="my-auto">
            <div class="alert alert-success" role="alert" style="padding-left: 25%">
                <div class="mainpage-text animated-text">
                    <h4 class="alert-heading">Скрипт был запущен! База Данных обновлена!</h4>
                </div>
            </div>
        </div>
</template>
...
export default {
    ...
    mounted() {
        axios({
            method: "get",
            url: this.base_backend_url + "/script",
            headers: {
